...
public class TrainGenerator implements Runnable {
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(TrainGenerator.class.getName());
    ...
    public TrainGenerator(Tunnel tunnel) {
        this.tunnel = tunnel;
    }
    ...
    public void run() {
        try {
            for (int i = 1; i <= HOW_MUCH_TRAINS_WILL_BE_GENERATED
                    ; i++) {
    ...
    public Train createTrain(int pullOfNumbersTrain, int tunnel) {
        ...
        return new Train(countRailWagons, trainNumber, Locations.values()[random.nextInt(2)], tunnel);
    }
